...
SELECT 
	*
FROM USUARIO U
INNER JOIN ROTEIRO_VIAGEM R
ON U.NICKNAME = R.AUTOR
WHERE R.NOME_ROTEIRO = 'Roteiro 1' AND U.NICKNAME = 'LeoF' AND U.SENHA = '123456789';
...
SELECT
	U.NICKNAME,U.NOME,R.NOME_ROTEIRO
FROM USUARIO U 
LEFT JOIN ROTEIRO_VIAGEM R ON U.NICKNAME = R.AUTOR
WHERE UPPER(U.NOME) LIKE 'L%';
...
WHERE UPPER(P.NOME_CAMPEONATO) = 'MUNDIAL DE CLUBES'
...
WHERE UPPER(A.NOME) =  'DANIEL ALVES';
...
SELECT 
	AUTOR_ROTEIRO,NOME_ROTEIRO, SUM(TOTAL)
FROM 
( 	SELECT r.AUTOR_ROTEIRO,r.NOME_ROTEIRO,p.PRECO*r.QTD_INGRESSOS AS TOTAL, 'PARTIDA' AS TIPO FROM ROTEIRO_PARTIDA r
  	INNER JOIN PARTIDA p ON r.PARTIDA = p.CODIGO_PARTIDA 
...
UNION ALL
	...
	INNER JOIN TRANSPORTE t ON r.NOME_EMPRESA_TRANSPORTE = t.NOME_DA_EMPRESA
	AND r.CODIGO_POSTALORIGEM = t.CODIGO_POSTALORIGEM AND r.CODIGO_POSTALDESTINO = t.CODIGO_POSTALDESTINO)
...
(SELECT
	...
	FROM 
		(SELECT 
			r.AUTOR_ROTEIRO,
			r.NOME_ROTEIRO,
			p.PRECO*r.QTD_INGRESSOS AS TOTAL,
			'PARTIDA' AS TIPO
		FROM ROTEIRO_PARTIDA r
		INNER JOIN PARTIDA p
			ON r.PARTIDA = p.CODIGO_PARTIDA 
		UNION ALL
		SELECT
			r.AUTOR_ROTEIRO,
			r.NOME_ROTEIRO,
			p.PRECO*r.QTD_INGRESSOS AS TOTAL,
			'PONTO' AS TIPO
		FROM ROTEIROPONTO_TURISTICO r
		INNER JOIN PONTO_TURISTICO p
			ON r.NOME_PONTO = p.NOME AND r.CODIGO_POSTALPONTO = p.CODIGO_POSTAL 
		UNION ALL 
		
		SELECT 
			r.AUTOR_ROTEIRO,
			r.NOME_ROTEIRO,
			h.PRECO*r.QTD_INGRESSOS AS TOTAL,
			'HOSPEDAGEM' AS TIPO
		FROM ROTEIRO_HOSPEDAGEM r
		INNER JOIN HOSPEDAGEM h 
			ON r.NOME_HOSPEDAGEM = h.NOME AND r.CODIGO_POSTAL = h.CODIGO_POSTAL
		UNION ALL
		SELECT 
			r.AUTOR_ROTEIRO,
			r.NOME_ROTEIRO,
			t.PRECO*r.QTD_INGRESSOS AS TOTAL,
			'TRANSPORTE' AS TIPO
		FROM ROTEIRO_TRANSPORTE r
		INNER JOIN TRANSPORTE t
			ON r.NOME_EMPRESA_TRANSPORTE = t.NOME_DA_EMPRESA
			AND r.CODIGO_POSTALORIGEM = t.CODIGO_POSTALORIGEM
			AND r.CODIGO_POSTALDESTINO = t.CODIGO_POSTALDESTINO)
...
SELECT 
	u1.NICKNAME,
	u1.NOME,
	t1.NOME_FANTASIA AS Nome_Time,
	c1.NOME_DO_CLUBE 
FROM USUARIO u1
INNER JOIN USUARIO_TORCE ut1
	ON u1.NICKNAME = ut1.USUARIO
INNER JOIN TIME t1 
	ON ut1.ID_TIME = t1.ID_TIME 
INNER JOIN CLUBE c1
	ON c1.ID_CLUBE = t1.ID_CLUBE
WHERE 
	(SELECT count(*) FROM
...
/**
 * 12. Selecionar para cada país os Pontos Turístico  com preço abaixo da média de seu país
 * 
 */
...
WHERE
	p.PRECO < 
	(SELECT AVG(po.PRECO) FROM PONTO_TURISTICO po WHERE po.PAIS = p.PAIS)
